...
int main() {
	...
	cout << "检验" << endl;
	int max = weightA;
	max = max > weightB ? max: weightB;
	max = max > weightC ? max: weightC;
...
> 注意1：switch语句中表达式类型只能是**整型**或者**字符型**
...
![](H:\OneDrive\OneDrive - ZTE\C++\匠心精作C++从0到1入门编程-学习编程不再难资料\第1阶段C++ 匠心之作 从0到1入门资料\学习C++不再难资料\C++基础入门讲义\assets\QQ截图20221215194319.png)
...
int main() {
	...
	//RAND_MAX定义在stdlib.h, 其值为3276768
	//int num = rand() % 100 + 1;  //这种方法重新运行程序都是相同的结果

	///需要添加随机数种子，作用：利用当前系统时间生成随机数
	srand((unsigned int)time(NULL));
	int num = rand() % 100 + 1;
	cout << "让我们来玩猜数游戏吧~" << endl;
	...
	while (!flag)
	{
		...
		{
			cout << "你猜的数太大了，请重新输入：" << endl;
		}
		else
		{
			cout << "你猜的数太小了，请重新输入：" << endl;
		}
	}
...
数组的元素个数为： 10
数组首地址为： -1984629064
数组中第一个元素地址为： -1984629064
数组中第二个元素地址为： -1984629060
请按任意键继续. . .
```
...
数组逆转后为：
4 5 2 3 1
H:\C++\C++项目\Project1 C++书写HelloWorld\Project1 C++书写HelloWorld\Debug\Project1 C++书写HelloWorld.exe (进程 6436)已 退出，代码为 0。
按任意键关闭此窗口. . .
```
...
二维数组行数： 2
二维数组列数： 3
二维数组首地址：00000023CE7DF508
二维数组第一行地址：00000023CE7DF508
二维数组第二行地址：00000023CE7DF514
二维数组第一个元素地址：00000023CE7DF508
二维数组第二个元素地址：00000023CE7DF50C
```
...
void swap(int num1, int num2)
{
	...
	//return ; 当函数声明 void 时候，不需要返回值，可以不写return
}
...
笔记：根据实验结果，在64位操作系统下与上述总结不符，很明显，整型指针10个字节，char型指针、float型指针和double型指针是8个字节。
